...
Each testcase must still follow flow pipeline first:
`flow-resolution` -> `flow.adapted` -> generated script -> execution.
...
Each agent:
  ...
  - Generates script from flow.adapted.json
  - Executes generated script for the testcase
